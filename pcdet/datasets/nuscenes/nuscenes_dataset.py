...
class NuScenesDataset(DatasetTemplate):
    ...
    def crop_image(self, input_dict):
        ...
        #print("np.array(images).shape_2",np.array(crop_images).shape)
        return input_dict
    ...
    def load_camera_info(self, input_dict, info):
        ...
        #print("np.array(images).shape",np.array(images).shape)
...
####################################################################################################
    ...
    def _load_data(self,index,input_dict):
        ...
        #print("image_paths",len(input_dict["image_paths"]))
...
#################################################################################################################
    def __getitem__(self, index):
        ...
        same_aug_pre=True
        pc_hm_list = []
        #print("data_dict['camera_imgs']",len(data_dict['camera_imgs']))
        #print(info["cams"].items())
        idx = 0
        for _, camera_info in info["cams"].items():
            ...
            s = max(img.shape[0], img.shape[1]) * 1.0 if not not_max_crop else np.array([img.shape[1], img.shape[0]], np.float32)
        ...
        #print("pc_hm",data_dict['pc_hm'].shape)
