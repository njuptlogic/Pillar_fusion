...
class NuScenesDataset(DatasetTemplate):
    ...
    def _get_aug_param(self, c, s, width, height, disturb=False):
        if (not not_rand_crop) and not disturb:
            ...
            c[0] += s * np.clip(temp, -1*cf, 1*cf)
            c[1] += s * np.clip(np.random.randn()*cf, -1*cf, 1*cf)
    ...
    def _process_pc(self, pc_2d, pc_3d, img, inp_trans, out_trans, camera_info):    
        ...
        if pc_roi_method == "pillars":
          pillar_wh = self.create_pc_pillars(img, camera_info, pc_2d, pc_3d, inp_trans, out_trans)    
    ...
    def create_pc_pillars(self, img, camera_info, pc_2d, pc_3d, inp_trans, out_trans):
        ...
        for i, center in enumerate(pc_3d[:3,:].T):
          ...
          box_2d = project_to_image(box_3d, camera_info['calib']).T  # [2x8]        
    ...
    def _load_pc_data(self, img, camera_info, inp_trans, out_trans, input_dict ,flipped=0):
        #img_height, img_width = img.shape[0], img.shape[1]
        ...
        radar_pc = np.array(camera_info.get('radar_pc', None))
        ...
        pc_2d, mask = map_pointcloud_to_image(radar_pc, np.array(camera_info['camera_intrinsic_2']), img_shape=(W,H))
        pc_3d = radar_pc[:,mask] 
        ...
        pc_2d, pc_3d, pc_dep = self._process_pc(pc_2d, pc_3d, img, inp_trans, out_trans, camera_info)
    ...
    def __getitem__(self, index):
        ...
        idx = 0
        for _, camera_info in info["cams"].items():
            #img = data_dict['camera_imgs'][idx]
            img = input_dict['camera_imgs'][idx]
            '''
            img_root = '/root/lanyun-tmp/Pillar_fusion/save_6'
            # 保存图像
            camera_filename = f'{index}_{idx}.png'  # 使用计数器来命名图片
            camera_path = os.path.join(img_root, camera_filename)
            # 显示图像（如果需要）
            plt.imshow(img)
            plt.axis('off')  # 关闭坐标轴
            plt.savefig(camera_path, bbox_inches='tight', pad_inches=0)  # 保存图像到指定目录
            '''
            
            width, height = input_dict["ori_shape"]
            ## Get center and scale from image
            c = np.array([width / 2., height / 2.], dtype=np.float32)
            s = max(height, width) * 1.0 if not not_max_crop else np.array([width, height], np.float32)
            ...
            if pointcloud:
                pc_2d, pc_N, pc_dep, pc_3d = self._load_pc_data(img,camera_info, trans_input, trans_output, input_dict, flipped=0)
                #print("pc_dep",pc_dep)
                #print("pc_dep_shape",pc_dep.shape)
                input_dict.update({ 'pc_2d': pc_2d,
                       'pc_3d': pc_3d,
                       'pc_N': pc_N, 
                       'pc_dep': pc_dep })
            ...
            input_dict['pc_hm_one'] = np.zeros((len(pc_feat_lvl), output_h, output_w), np.float32)
            calib = self._get_calib(camera_info, width, height)
            num_objs = min(len(camera_info['annotations']), self.max_objs)
            for k in range(num_objs):
                ann = camera_info['annotations'][k]
                ...
                pc_dep_to_hm(input_dict['pc_hm_one'], input_dict['pc_dep'], ann['depth'], bbox, dist_thresh)
            '''
            input_dict['pc_hm_one'] = np.moveaxis(input_dict['pc_hm_one'], 0, -1)
            ten_np = input_dict['pc_hm_one'].mean(axis=-1)
            gray_image = (ten_np-ten_np.min())/(ten_np.max()-ten_np.min())
            ...
            ###############################################
            '''
            if input_dict['pc_hm_one'] is None:
                print("none pc hm")
            '''
            crop_info = input_dict['img_process_infos'][idx]
            idx = idx+1
            W,H = width, height
            resize = crop_info[0]
            crop = crop_info[1]
            ...
            # 获取最小值和最大值
            min_val = np.min(input_dict['pc_hm_one'])
            max_val = np.max(input_dict['pc_hm_one'])

            # 如果最大值和最小值相等，则设置为零
            if max_val == min_val:
                input_dict['pc_hm_one'] = np.zeros_like(input_dict['pc_hm_one'], dtype=np.uint8)
            else:
                # 执行归一化
                input_dict['pc_hm_one'] = np.uint8(255 * (input_dict['pc_hm_one'] - min_val) / (max_val - min_val))
                #input_dict['pc_hm_one'] = np.uint8(input_dict['pc_hm_one'])
            
            # 将 NumPy 数组转换为 PIL 图像
            pc_hm_img = np.moveaxis(input_dict['pc_hm_one'], 0, -1)  # 将通道维度移动到最后
            input_dict['pc_hm_one'] = Image.fromarray(pc_hm_img)
            # 设置断点
            #set_trace()  # 进入调试模式
            #pdb.set_trace()  # 这里设置断点

            '''
            pc_img = np.array(input_dict['pc_hm_one'])
            plt.imshow(pc_img)
            plt.axis('off')  # 关闭坐标轴
            save_root = '/root/lanyun-tmp/Pillar_fusion/save_3'
            image_filename = f'{index}_{idx}.png'  # 使用计数器来命名图片
            image_path = os.path.join(save_root, image_filename)
            plt.savefig(image_path, bbox_inches='tight', pad_inches=0)  # 保存图像到指定目录
            '''
            
            input_dict['pc_hm_one'] = input_dict['pc_hm_one'].resize((newW, newH))
            input_dict['pc_hm_one'] = input_dict['pc_hm_one'].crop(crop)
            
            '''
            pcimg = np.array(input_dict['pc_hm_one'])
            plt.imshow(pcimg)
            plt.axis('off')  # 关闭坐标轴
            save_root = '/root/lanyun-tmp/Pillar_fusion/save_4'
            image_filename = f'{index}_{idx}.png'  # 使用计数器来命名图片
            image_path = os.path.join(save_root, image_filename)
            plt.savefig(image_path, bbox_inches='tight', pad_inches=0)  # 保存图像到指定目录
            '''

            input_dict['pc_hm_one'] = np.array(input_dict['pc_hm_one'])

            # 恢复原始张量数据，执行逆归一化
            input_dict['pc_hm_one'] = (input_dict['pc_hm_one'] / 255.0) * (max_val - min_val) + min_val
            '''
            pc_2 = input_dict['pc_hm_one']
            min_pc = np.min(pc_2)
            max_pc = np.max(pc_2)

            if max_pc == min_pc:
                pc_2 = np.zeros_like(pc_2, dtype=np.uint8)
            else:
                # 执行归一化
                pc_2 = np.uint8(255 * (pc_2 - min_pc) / (max_val - min_pc))
            plt.imshow(pc_2)
            plt.axis('off')  # 关闭坐标轴
            save_root = '/root/lanyun-tmp/Pillar_fusion/save_5'
            image_filename = f'{index}_{idx}.png'  # 使用计数器来命名图片
            image_path = os.path.join(save_root, image_filename)
            plt.savefig(image_path, bbox_inches='tight', pad_inches=0)  # 保存图像到指定目录
            '''
            pc_hm_list.append(input_dict['pc_hm_one'])
        data_dict['pc_hm'] = pc_hm_list
