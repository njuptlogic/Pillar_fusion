...
def extract_radar_point_clouds(self, index, max_sweeps=10):
    """
    提取雷达点云数据，并返回合并后的雷达点云信息。

    Args:
        self (NuScenesDataset): NuScenes 数据集类的实例。
        index (int): 需要处理的样本索引。
        max_sweeps (int): 需要提取的最大扫描次数。

    Returns:
        np.ndarray: 合并后的雷达点云数据，格式为 (18, N)。
    """
    
    info = self.infos[index]  # 获取当前帧的信息

    # 确保 `radar_sweeps` 不为空
    if 'radar_sweeps' not in info or len(info['radar_sweeps']) == 0:
        return np.zeros((18, 0), dtype=np.float32)  # 18 个字段，空数据返回 (18, 0)

    radar_sweeps = info['radar_sweeps']
    all_radar_points = []

    for radar_channel in radar_sweeps:
        sweeps = radar_channel['sweeps']
        
        channel_radar_points = []

        for i, sweep in enumerate(sweeps[:max_sweeps]):  # 遍历 sweeps
            radar_path = self.root_path / sweep['radar_path']

            # 使用 NuScenes 自带的 RadarPointCloudWithVelocity 解析雷达点云
            radar_pc, _ = RadarPointCloudWithVelocity.from_file_multisweep(
                self.nusc, 
                info, 
                chan=radar_channel['channel'],  # 使用当前雷达通道
                ref_chan=radar_channel['channel'],  # 使用雷达通道本身作为参考通道
                nsweeps=max_sweeps
            )

            # 确保数据维度是 (18, N)
            if radar_pc.points.shape[0] != 18:
                print(f"⚠️ WARNING: Unexpected number of fields {radar_pc.points.shape[0]} in radar data.")
                continue
          
            # 存储当前 sweep 的点云
            channel_radar_points.append(radar_pc.points)

        # **合并当前通道的 sweeps 数据**
        if len(channel_radar_points) > 0:
            channel_radar_points = np.concatenate(channel_radar_points, axis=1)  # (18, N)
            all_radar_points.append(channel_radar_points)

    # **合并所有雷达通道的数据**
    if len(all_radar_points) > 0:
        final_radar_pc = np.concatenate(all_radar_points, axis=1)  # (18, N)
    else:
        final_radar_pc = np.zeros((18, 0), dtype=np.float32)  # 处理空数据情况

    return final_radar_pc
...
def extract_radar_point_clouds(self, index, max_sweeps=10):
    ...
    Returns:
        np.ndarray: 合并后的雷达点云数据，格式为 (x, y, z, velocity, time_lag)。
    """
    ...
    info = self.infos[index]  # 获取当前帧的信息
    radar_sweeps = info['radar_sweeps']  # 获取 radar_sweeps
    ...
    for radar_channel in radar_sweeps:
        ...
        for i, sweep in enumerate(sweeps[:max_sweeps]):  # 遍历 sweeps
            ...
            # 读取雷达点云数据（根据实际情况处理字段数量）
            radar_points = np.fromfile(str(radar_path), dtype=np.float32, count=-1)
            
            # 动态调整 reshape，假设每个点有5个字段
            num_fields = 5  # 根据 .pcd 文件分析，每个点有5个字段
            #radar_points = radar_points.reshape(-1, num_fields)
            radar_points = radar_points.reshape(num_fields, -1)
            
            # 提取 [x, y, z, vx, vy]（假设每个点有这几个字段）
            #radar_points = radar_points[:, :5]  # 只取 x, y, z, vx, vy
            
            # 处理点云数据：移除自车周围点
            # radar_points = remove_ego_points(radar_points)
            
            # 进行坐标变换
            if sweep['transform_matrix'] is not None:
                num_points = radar_points.shape[0]
                radar_points[:, :3] = sweep['transform_matrix'].dot(
                    np.vstack((radar_points[:, :3].T, np.ones(num_points)))
                )[:3, :].T
            
            # 计算时间戳延迟
            #time_lag = sweep['time_lag'] * np.ones((radar_points.shape[0], 1))
            time_lag = sweep['time_lag'] * np.ones((1, radar_points.shape[1]))  # (1, N)
            # 存储当前 sweep 的点云
            channel_radar_points.append(radar_points)
            channel_time_lags.append(time_lag)

        # 合并当前通道的 sweeps 数据
        if len(channel_radar_points) > 0:
            channel_radar_points = np.concatenate(channel_radar_points, axis=1)
            channel_time_lags = np.concatenate(channel_time_lags, axis=1)
            
            # 合并到所有通道的 radar 数据
            all_radar_points.append(channel_radar_points)
            all_time_lags.append(channel_time_lags)

    # 合并所有雷达通道的数据
    if len(all_radar_points) > 0:
        all_radar_points = np.concatenate(all_radar_points, axis=1)
        all_time_lags = np.concatenate(all_time_lags, axis=1)

        # 组合最终点云数据
        final_radar_pc = np.concatenate((all_radar_points, all_time_lags), axis=0)  # (num_fields+1, N)
    else:
        #final_radar_pc = np.zeros((6, 0), dtype=np.float32)  # 空数据
        final_radar_pc = np.zeros((num_fields + 1, 0), dtype=np.float32)


    # 将点云数据与偏移量进行处理（如果设置了偏移量）
    
    if self.opt.pc_z_offset != 0:
        final_radar_pc[:, 1] -= self.opt.pc_z_offset  # 对 y 坐标进行偏移
...
class NuScenesDataset(DatasetTemplate):
    ...
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        '''
        print("####################################################")
        print("successfully loading....1010101010101010")
        print("####################################################")
        split_names = {
        ...
        self.data_dir = data_dir  # 赋值为传入的路径
        self.data_dir = os.path.join(data_dir, 'nuscenes','v1.0-trainval')
        print('Dataset version', dataset_version)
    
        anns_dir = 'annotations'
        if radar_sweeps > 1:
            anns_dir += '_{}sweeps'.format(radar_sweeps)

        if dataset_version == 'test':
            ann_path = os.path.join(self.data_dir, anns_dir, 'test.json')
        else:
            ann_path = os.path.join(self.data_dir, anns_dir, '{}.json').format(split_name)
            print("####################################################")
            print("successfully loading....1212121212121212121")
            print("####################################################")
            self.coco = coco.COCO(ann_path)
            self.images = self.coco.getImgIds()
            self.img_dir = img_dir
        print("####################################################")
        print("successfully loading....9999999999999999999999999")
        print("####################################################")
        #####################################
        '''
...
####################################################################################################
    ...
    def _load_pc_data(self, img, info, inp_trans, out_trans, flipped=0):
        img_height, img_width = img.shape[0], img.shape[1]
        ...
        pc_2d, mask = map_pointcloud_to_image(radar_pc, np.array(info['camera_intrinsic_2']), img_shape=(info['width'],info['height']))
        pc_3d = radar_pc[:,mask] 
...
#################################################################################################################
    def __getitem__(self, index):
        ...
        if pointcloud:
          pc_2d, pc_N, pc_dep, pc_3d = self._load_pc_data(img,info, trans_input, trans_output, flipped=0)
...
########################################################################################################
    ...
    def create_groundtruth_database(self, used_classes=None, max_sweeps=10, with_cam_gt=False, share_memory=False):
        ...
        for idx in tqdm(range(len(self.infos))):
            ...
            radar_points = extract_radar_point_clouds(self, idx, max_sweeps=10)
            #######################################################
