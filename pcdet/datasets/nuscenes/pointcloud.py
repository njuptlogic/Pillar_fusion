...
def pc_dep_to_hm(pc_hm, pc_dep, dep, bbox, dist_thresh):
    ...
    roi = pc_dep[:, bbox_int[1]:bbox_int[3]+1, bbox_int[0]:bbox_int[2]+1]
    pc_dep = roi[pc_feat_channels['pc_dep']]
    pc_vx = roi[pc_feat_channels['pc_vx']]
    pc_vz = roi[pc_feat_channels['pc_vz']]
    ...
    #   nonzero_pc_dep = np.exp(-pc_dep[nonzero_inds])
      ...
      if len(pc_dep_match) > 0:
        ...
        w = bbox[2] - bbox[0]
        w_interval = hm_to_box_ratio*(w)
        w_min = int(ct[0] - w_interval/2.)
        w_max = int(ct[0] + w_interval/2.)
        
        h = bbox[3] - bbox[1]
        h_interval = hm_to_box_ratio*(h)
        h_min = int(ct[1] - h_interval/2.)
        h_max = int(ct[1] + h_interval/2.)

        pc_hm[pc_feat_channels['pc_dep'],
               h_min:h_max+1, 
               w_min:w_max+1+1] = dist
        pc_hm[pc_feat_channels['pc_vx'],
               h_min:h_max+1, 
               w_min:w_max+1+1] = vx
        pc_hm[pc_feat_channels['pc_vz'],
               h_min:h_max+1, 
               w_min:w_max+1+1] = vz
